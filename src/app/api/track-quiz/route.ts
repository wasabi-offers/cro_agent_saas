...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const event of events) {
      ...
      if (!funnelSlug || !eventType || !eventUserId) continue;
      ...
      if (!funnel) {
        console.error(`[track-quiz] Funnel not found: ${funnelSlug}`);
        continue;
      }
      ...
      if (eventType === "quiz_start") {
        const { data: newSessionId, error: startErr } = await supabase.rpc(
          "start_quiz_session",
          {
        ...
        if (startErr) {
          console.error(
            "[track-quiz] start_quiz_session error:",
            startErr.message
          );
        } else {
      ...
      // Find existing quiz session if needed
      ...
      const eventData: Record<string, unknown> = {
        ...
        interaction_data: {
          ...
          dropoff_reason: event.dropoff_reason,
      ...
      if (eventType === "answer_click" && quizSessionId) {
        ...
        if (ansErr)
          console.error(
            "[track-quiz] record_quiz_answer error:",
            ansErr.message
          );
      ...
      if (eventType === "quiz_complete" && quizSessionId) {
        ...
        if (compErr)
          console.error(
            "[track-quiz] complete_quiz_session error:",
            compErr.message
          );
      }
      ...
      if (eventType === "quiz_abandon" && quizSessionId) {
        ...
        if (abErr)
          console.error(
            "[track-quiz] abandon_quiz_session error:",
            abErr.message
          );
      }

      // ── Update answer stats (daily aggregate) ──
      if (eventType === "answer_click" && event.answer_id) {
        ...
        if (existing) {
          ...
          await supabase.from("quiz_answer_stats").insert({
            funnel_id: funnelId,
    ...
    console.error("[track-quiz] Fatal error:", message);
    return NextResponse.json(
      { error: "Failed to track quiz events", details: message },
