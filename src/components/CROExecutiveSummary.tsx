...
export default function CROExecutiveSummary({ results, source }: CROExecutiveSummaryProps) {
  ...
  const scoreColor = avgScore >= 80 ? "text-[#06B6D4]" : avgScore >= 60 ? "text-[#f59e0b]" : "text-[#EF4444]";
  const scoreBg = avgScore >= 80 ? "bg-[#06B6D4]/10 border-[#06B6D4]/20" : avgScore >= 60 ? "bg-[#f59e0b]/10 border-[#f59e0b]/20" : "bg-[#EF4444]/10 border-[#EF4444]/20";
  ...
  return (
    <div className="bg-[#0B0F19] border border-[#1E293B] rounded-2xl p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[16px] font-semibold text-[#F8FAFC]">Executive Summary</h3>
        {source && (
          <span className="text-[10px] px-2 py-1 rounded bg-[#1E293B] text-[#94A3B8]">
            Source: {source}
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className={`rounded-xl border p-4 ${scoreBg}`}>
          <div className="text-[12px] text-[#94A3B8] mb-1">Overall Health</div>
        ...
        <div className="md:col-span-2 space-y-2">
          ...
          <ul className="space-y-1.5">
            {criticalInsights.map((insight, i) => (
              <li key={i} className="text-[12px] text-[#94A3B8] line-clamp-2">
      ...
      {topQuickWins.length > 0 && (
        <div className="mt-4 pt-4 border-t border-[#1E293B]">
          <div className="flex items-center gap-2 text-[12px] font-semibold text-[#06B6D4] mb-2">
            <Zap className="w-4 h-4" />
            Quick Wins (Low Effort)
          </div>
          <ul className="space-y-1.5">
            {topQuickWins.map((insight, i) => (
              <li key={i} className="text-[12px] text-[#F8FAFC] line-clamp-2">
                • {insight}
              </li>
            ))}
          </ul>
