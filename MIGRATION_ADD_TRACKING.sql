-- ============================================
-- MIGRATION: ADD ADVANCED TRACKING TABLES
-- ============================================
-- Run this SQL in your Supabase SQL Editor to add tracking tables
-- This migration adds comprehensive CRO tracking capabilities
...
-- ============================================
-- 3. HEATMAP DATA (Aggregated for performance)
-- ============================================
...
-- ============================================
-- 5. FUNNEL PROGRESSION (Enhanced)
-- ============================================
CREATE TABLE IF NOT EXISTS funnel_progressions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id VARCHAR NOT NULL REFERENCES tracking_sessions(session_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_sessions_first_seen ON tracking_sessions(first_seen_at);
CREATE INDEX IF NOT EXISTS idx_sessions_device ON tracking_sessions(device_type);
CREATE INDEX IF NOT EXISTS idx_sessions_utm_source ON tracking_sessions(utm_source);
CREATE INDEX IF NOT EXISTS idx_sessions_converted ON tracking_sessions(converted);

-- Events
CREATE INDEX IF NOT EXISTS idx_events_session_id ON tracking_events(session_id);
...
CREATE INDEX IF NOT EXISTS idx_events_path ON tracking_events(path);
CREATE INDEX IF NOT EXISTS idx_events_funnel ON tracking_events(funnel_id);
CREATE INDEX IF NOT EXISTS idx_events_created ON tracking_events(created_at);

-- Heatmap
CREATE INDEX IF NOT EXISTS idx_heatmap_path_type ON heatmap_data(page_path, event_type, date);
CREATE INDEX IF NOT EXISTS idx_heatmap_date ON heatmap_data(date);

-- Conversions
CREATE INDEX IF NOT EXISTS idx_conversions_session ON conversion_events(session_id);
CREATE INDEX IF NOT EXISTS idx_conversions_name ON conversion_events(event_name);

-- Funnel progressions
CREATE INDEX IF NOT EXISTS idx_progressions_session ON funnel_progressions(session_id);
CREATE INDEX IF NOT EXISTS idx_progressions_funnel ON funnel_progressions(funnel_id);
CREATE INDEX IF NOT EXISTS idx_progressions_completed ON funnel_progressions(completed);
...
-- CTA performance
CREATE OR REPLACE VIEW cta_performance AS
SELECT
  click_element_text,
  click_element_class,
...
-- Funnel analytics
CREATE OR REPLACE VIEW funnel_analytics_view AS
SELECT
  funnel_id,
  funnel_step_name,
  funnel_step_order,
...
-- MIGRATION COMPLETE
-- ============================================
-- Your tracking tables are now ready!
-- The system will now capture:
-- - Page views, clicks, CTA clicks
-- - Scroll depth, mouse movements
-- - Form interactions
-- - Rage clicks, dead clicks, exit intent
-- - Time on page, engagement
-- - Device info, UTM parameters
